...
import com.sleekydz86.store.global.exceptions.resp.ExceptionResponse
...
class GlobalExceptionAdvice {
    ...
    fun handleMethodArgumentNotValidException(
        ...
        log.error("잘못된 파라미터 값이 들어왔습니다. 요청: {} {}", request.method, request.requestURI)
    ...
    fun handleDtoValueEmpty(exception: HttpMessageNotReadableException): ResponseEntity<ExceptionResponse> {
        val message =
            if (exception.cause?.javaClass?.simpleName == "KotlinInvalidNullException") {
                "요청 필드 값으로 null 값이 오면 안됩니다."
            } else {
                "요청을 역직렬화 하는과정에서 예외가 발생했습니다."
            }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
